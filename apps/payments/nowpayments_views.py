...
def connect_nowpayments(request, slug):
    ...
    try:
        ...
        messages.error(request, "NOWPayments could not verify the API key. No payment route was activated.")
        return redirect("seller_section", slug=slug, section="providers")
    _seller_audit(
        request,
        merchant,
...
def replace_nowpayments_credentials(request, slug, connection_uuid):
    ...
    try:
        ...
        messages.error(request, "The replacement API key was not verified. Existing working credentials were preserved.")
        return redirect("seller_section", slug=slug, section="providers")
...
def test_nowpayments_connection(request, slug, connection_uuid):
    ...
    try:
        ...
        messages.error(request, "NOWPayments connection check failed. Routing was disabled if no working credentials remained.")
        return redirect("seller_section", slug=slug, section="providers")
